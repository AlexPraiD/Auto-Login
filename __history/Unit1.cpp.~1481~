...
int Count()
{
	Form1->Label4->Caption="Количество записей: "+IntToStr(Form1->ADOTable1->RecordCount);
...
void __fastcall TForm1::Timer1Timer(TObject *Sender)
{
	...
	if(sec==2)
		{
			Tab();
		CopyThis->Text=password;          //пароль
			CopyThis->SelectAll();
			CopyThis->CopyToClipboard();
		Contrl_V();
			keybd_event(VK_RETURN, 0,0,0);  //Ввод
	...
	if(sec==1)
		{
			if(buf=="")
				{
					CopyThis->Text="Clear";    // зачистка буфера от данных
						CopyThis->SelectAll();
						CopyThis->CopyToClipboard();
				}
			else
				{
					CopyThis->Text=buf;
						CopyThis->SelectAll();  //занесение данных назад в буфер
						CopyThis->CopyToClipboard();
						CopyThis->Text="";
				}
        }
...
void __fastcall TForm1::FormCreate(TObject *Sender)
{
	if(FileExists("ALog.mdb"))
		...
		Panel8->Left=446;
			...
			if(CheckBox4->Checked==true)               //включение/выключение настроек анимации
		...
		{
			ShowMessage("Оопс, файл базы не найден!");
			Form1->Close();
		}
}
//---------------------------------------------------------------------------
					   ...
					   if(Access==1)
							{
								...
								Report("Использование", login, ADOTable1->FieldByName("Site")->AsString);
										Label4->Caption="Количество записей:"+IntToStr(ADOTable1->RecordCount);
...
void __fastcall TForm1::Button13Click(TObject *Sender)
{
	if((Edit1->Text!="")||(Edit2->Text!="")||(Edit8->Text!=""))
		{
		Button6Click(Sender);   //выключение фильтра в таблице
		ADOTable1->First();
			ADOTable1->Append();
			...
			ADOTable1->Post();
			OffFiltreFormTwo();
				Report("Добавление", Edit2->Text, Edit1->Text);
				Numb();
...
void __fastcall TForm1::Button4Click(TObject *Sender)
{
	if(ADOTable4->Locate("Password",XOR_Crypt(Edit4->Text),TLocateOptions() << loCaseInsensitive))
		{
			UnregisterHotKey(Form1->Handle,0x00F); // отключение горячих главиш
			bool RHKret = RegisterHotKey(Form1->Handle,0x00F, MOD_ALT, VK_RETURN);  // вkл горячих клавиш
				if (RHKret)
					{
		...
		{
			Image4->Visible=true;
				Slee_p(1000); //задержка
			Image4->Visible=false;                                      //access denied
			Edit4->Clear();
			Report("Неудачный вход", "--", "--");
		}
}
//---------------------------------------------------------------------------
	...
	if(ADOTable1->RecordCount!=0)
		{
			Button21Click(Sender);
			OffFiltreFormTwo();
				Report("Редактирование", Edit11->Text, Edit10->Text);
			ADOTable1->Edit();
	...
	ADOTable1->FieldByName("Password")->AsString!="Test")
		{
		 Button6Click(Sender);  //Выключение фильтра в таблице
			Button24Click(Sender);
			OffFiltreFormTwo();
				Report("Удаление", ADOTable1->FieldByName("Login")->AsString,
          ...
          int Y)
{
	if(ADOTable1->FieldByName("Note")->AsString=="")
		DBGrid1->Hint="Примечание: Отсуствует";
	else
		DBGrid1->Hint="Примечание: "+XOR_Crypt(ADOTable1->FieldByName("Note")->AsString);
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Timer4Timer(TObject *Sender)
{
	...
	AnsiString now_prog, now_prog2;                      //Сканирование имён активных окон
			HWND fore = ::GetForegroundWindow();
			TCHAR szTitle[MAX_PATH] = {0};
			::SendMessage(fore, WM_GETTEXT, MAX_PATH, LPARAM(szTitle));
			now_prog = String(szTitle);
...
void __fastcall TForm1::Timer6Timer(TObject *Sender)
{
	...
	GroupBox4->Top=Form1->Height-48;   // Подпись с низу с мылом
	Label5->Left=Form1->Width-205;
}
//---------------------------------------------------------------------------
